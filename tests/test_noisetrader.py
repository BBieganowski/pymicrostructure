...
from microstructpy.traders.noise import NoiseTrader  # Assuming the NoiseTrader class is in trader.py
...
def test_noise_trader_custom_initialization(mock_market):
    ...
    trader = NoiseTrader(market=mock_market, submission_rate=0.5, volume_size=custom_volume)
    assert trader.market == mock_market
