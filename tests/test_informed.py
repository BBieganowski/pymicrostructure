...
def test_dummy_informed_trader_template():
    dummy_trader_template = DummyInformedTrader()
    mock_market = MagicMock(spec=ContinuousDoubleAuction(initial_fair_price=1000))
    
    trader = dummy_trader_template.create(mock_market)
    
    assert isinstance(trader, BaseInformedTrader)
    assert trader.name == "Dummy Informed Trader"
    assert trader.fair_price_strategy(trader) == 1050
    assert trader.max_inventory == 1000
    assert callable(trader.volume_strategy)
