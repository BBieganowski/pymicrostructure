...
def test_base_market_maker_initialization(mock_market, mock_fair_price_strategy, mock_volume_strategy, mock_spread_strategy):
    mm = BaseMarketMaker(
        market=mock_market,
...
def test_base_market_maker_update(mock_market, mock_fair_price_strategy, mock_volume_strategy, mock_spread_strategy):
    mm = BaseMarketMaker(
        market=mock_market,
        fair_price_strategy=mock_fair_price_strategy,
        volume_strategy=mock_volume_strategy,
...
def test_kyle_market_maker(mock_fair_price, mock_market):
    ...
    mock_fair_price.assert_called_once_with(window=10, aggressiveness=2)
